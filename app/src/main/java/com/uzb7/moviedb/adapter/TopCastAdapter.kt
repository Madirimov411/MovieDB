...
class TopCastAdapter(val list: ArrayList<Cast>):RecyclerView.Adapter<TopCastAdapter.TopCastViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TopCastViewHolder {
        return TopCastViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.item_top_billed_cast,parent,false))
    }
    ...
    override fun onBindViewHolder(holder: TopCastViewHolder, position: Int) {
        ...
        holder.apply {
            if(cast.profile_path!=null) { Glide.with(ivActors).load(CreateUrl.imageOpen(cast.profile_path)).into(ivActors) }
            name.text=cast.name
